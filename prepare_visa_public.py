...
def setup_arguments() -> Config:
    ...
    parser.add_argument('--split-type', default='1cls', type=str,
                        help='1cls, 2cls_highshot, 2cls_fewshot')
    parser.add_argument('--data-folder', default='./anomaly_detection/VisA_20220922', type=Path,
                        help='the path to downloaded VisA dataset')
    parser.add_argument('--save-folder', default='./anomaly_detection/VisA_20220922/VisA_pytorch/', type=Path,
                        help='the target path to save the reorganized VisA dataset')
    parser.add_argument('--split-file', default='./anomaly_detection/VisA_20220922/split_csv/1cls.csv', type=Path,
    ...
    try:
        with open(config.split_file, 'r', encoding='utf-8') as file:
            ...
            for row in csvreader:
                ...
                # --- Mask Copying (Conditional logic) ---
                ...
                if config.split_type == '1cls':
                    ...
                    if data_split == 'test' and label == 'bad':
                        msk_dst_path = save_folder / category / 'ground_truth' / label / mask_name
                        should_save_mask = True
                else:
                    # For other types, save all 'bad' masks, sorted by split
                    if label == 'bad':
                        msk_dst_path = save_folder / category / 'ground_truth' / data_split / label / mask_name
                        should_save_mask = True

                # Process and save the mask if needed
                if should_save_mask:
                    if not mask_path or not msk_src_path.exists():
                        print(f"Warning: Missing mask for bad sample: {image_path} (Expected at {msk_src_path})")
                        continue
